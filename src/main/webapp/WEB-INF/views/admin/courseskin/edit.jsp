...
<body>
    <div class="page-header">
    			<a href="${ctx}/admin/course/skin/list?ptype=${ptype}" class="backParent">
	...
	<div class="page-body editingBody">
		...
		<form class="form-horizontal" method="post" id="inputForm"
			action="${ctx}/admin/course/skin/save" name="form">
			...
			<section id="set-exam" class="section">
				   <div class="section">
					<label for="SchoolCover">皮肤封面</label> 
					<!--图片剪切-->
					...
					<div class="courseCover">
						<c:if test="${attId eq ''}">
						<img id="imgshow" name="imgshow" style="width: 170px; height: 100px;" src="${ctx}/resources/images/courseSkin-default.png"alt="" />
						</c:if>
						<c:if test="${attId ne ''}">
						<img id="imgshow" name="imgshow" style="width: 170px; height: 100px;" src="${ctx}/common/file/image/${attId}" alt="" />
						</c:if>
					</div>
				</div>
				...
				<div class="control-group">
					...
					<div class="controls controls-txt">
						<input value="${bean.fdSkinPath}" required="required"  class="input-block-level" id="fdSkinPath" name="fdSkinPath" type="text" style="color: rgb(169, 169, 169);">
					</div>
				</div>
				<div class="control-group">
					<label class="control-label">是否默认</label>
					<input value="${bean.fdDefaultSkin}"  id="fdDefaultSkin" name="fdDefaultSkin" type="hidden" >
					<ul class="nav nav-pills">
                        	<c:if test="${bean.fdDefaultSkin != null && bean.fdDefaultSkin=='true'}">
                        		<li class="active"><a data-toggle="tab" href="#true">是</a></li>
                        		<li><a data-toggle="tab" href="#false">否</a></li>
                        	</c:if>
                            <c:if test="${bean.fdDefaultSkin == null || bean.fdDefaultSkin=='false'}">
                        		<li><a data-toggle="tab" href="#true">是</a></li>
                        		<li class="active"><a data-toggle="tab" href="#false">否</a></li>
                        	</c:if>
                    </ul>
...
<script type="text/javascript">
	...
	$(function() {
		$('#inputForm a[data-toggle="tab"]').on('shown', function (e) {
	        var href = 	e.target.href.split("#").pop();
	        $("#fdDefaultSkin").val(href);
	    });
		//皮肤封面页
             ...
             jQuery("#upPagePic").uploadify({
                 ...
                 'onUploadSuccess' : function (file, data, Response) {
                     if (Response) {
                         ...
                         $.get("${ctx}/common/imageCut/page?imgSrcPath="+imgSrc+"&zoomWidth=170&zoomHeight=100&imgId="+objvalue.attId,function(data){
