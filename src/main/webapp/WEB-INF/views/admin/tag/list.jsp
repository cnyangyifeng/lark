...
<script type="text/javascript">
	...
	function pageNavClick(pageNo){
		var key = $('#fdKey').val();
		window.location.href=('${ctx}/admin/tag/list?pageNo='+pageNo+"&fdKey="+key);
	}
	...
	function goSearch() {
		window.location.href = '${ctx}/admin/tag/list?fdKey='+$("#fdKey").val();
		return;
	}
	    	...
	    	jalert("您确定要删除该标签？",function (){
	    		...
	    		$.ajax({
	    	    	...
	    	    	success:function(data){	
	    	    		if(data){
	    	    			jalert("选定标签在课程中有引用,确认删除？",checkDelete);
	    	    		}else{
	    	    			checkDelete();
	    	    		}
	    	    	}
	...
	function checkDelete(){
		...
		document.filterForm.action = '${ctx}/admin/tag/deleteAll?fdKey='+$("#fdKey").val();
...
<body>
	...
	<section class="section box-control">
		<div class="hd">
			...
			<a class="btn btn-primary" href="${ctx }/admin/tag/edit">添加</a>
				<div class="btn-group">
		...
		<ul class="nav list" id="materialList">
			 <j:iter items="${page.list}" var="bean" status="vstatus">
				<li data-id="${bean.fdId}">
					<a href="${ctx}/admin/tag/edit?fdId=${bean.fdId}"> 
					 <input type="checkbox" name="ids" value="${bean.fdId}"/>
					    <span class="title">${bean.fdName}&nbsp;&nbsp;${bean.fdDescription}</span>
					</a>
				</li>
			</j:iter> 
		</ul>
