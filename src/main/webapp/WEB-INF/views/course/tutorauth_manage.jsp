			  ...
			  <section class="col-left pull-left">
				...
				<section class="w790 pull-right" id="rightCont">
			        <div class="page-header">
		               <span class="muted">我正在看：按导师授权学习</span>
		                <div class="backHome">
			       ...
			       <section class="section mt20">
                    <div class="media">
                        ...
                        <c:if test="${param.teacherId==null || param.teacherId==''}">
                        	<input style="margin-bottom: 0;padding: 19px 10px;width: 728px;display: block;" type="text" required name="inputTutor" id="inputTutor"  class="autoComplete input-block ac_input" placeholder="授权学习的导师">
                ...
                <section class="section mt20">
                    <form action="#rightCont" id="formAddTeacher">
                        ...
                        <input type="text" required name="inputCourse" id="inputCourse" class="autoComplete input-block" placeholder="授权学习某个课程"/>
                        ...
                        <div class="btn-toolbar">
                            ...
                            <div class="btn-group btns-radio" data-toggle="buttons-radio">
							   ...
							   <c:if test="${param.order=='course'}">
								<button class="btn btn-large active" type="button" onclick="getDataOrderBy('1','course')">教师</button>
							   </c:if>
							   <c:if test="${param.order!='course'}">
								<button class="btn btn-large" type="button" onclick="getDataOrderBy('1','course')">教师</button>
							   </c:if>
...
function exportData(){
	...
	if(document.getElementById("selectAll").checked){
		 jalert("您确定要导出全部数据吗？",function(){
			  window.location.href="${ctx}/common/exp/getExpTutorAuth?isAll=all&teacherId="+teacherId+"&order="+order;
		 }); 
		 return;
	} else if(document.getElementById("selectCurrPage").checked){
		var current = $("#currentPage").val();
		jalert("您确定导出本页数据吗？",function(){
			window.location.href="${ctx}/common/exp/getExpTutorAuth?currentPage="+current+"&teacherId="+teacherId+"&order="+order;
		});
		return;
	} else{
...
/**********************initpage*********************************************/
    ...
    $(function(){
            ...
            submitHandler: function(form){
            	...
            	if($("#inputTutor").val()!=null&&$("#inputTutor").val()!=""){
            	 ...
            	 $.post("${ctx}/ajax/course/saveCourseParticipateAuth",{
            		'courseId':$("#course").val(),//课程id
                  	'teacher':$("#teacherIds").val(),//教师id//导师id
                  	'mentor':$("#teacherId").val()
               		  }).success(function(data){
               			if(data=='true'){
               				window.location.href="${ctx}/course/getSingleCourseAuthInfo?teacherId="+$("#teacherId").val()+"&order=createtime&fdType=15";
               			}else{
        ...
        $.ajax({
        	type: "post",
        	url: "${ctx}/ajax/course/getSingleTutorAuths",
        	data : {
        ...
        /****************************************导师信息***************************/
        $("#inputTutor").autocomplete("${ctx}/ajax/user/findByName",{
        	...
        	$("#inputTutor").val(item.name + '（' + item.mail + '），' + item.org  + item.department);
        ...
        $("#inputCourse").bind("focus",function(){
...
function confirmDel(){
 ...
 //删除所有授权信息
  function deleteAllCourseParticAuth(){
	 ...
	 $.ajax({
      	type: "post",
      	url: "${ctx}/ajax/course/deleteAuthByTutorId",
      	data : {
      		teacherId:"${param.teacherId}",
      		keyword:keyword
      	},
