						...
						<div class="btn-toolbar">
							...
							<div class="btn-group btns-radio" data-toggle="buttons-radio">
							   ...
							   <c:if test="${param.order=='fdname'}">
								<button class="btn btn-large active" type="button" onclick="findeCoursesByKey('1','fdname')">导师</button>
							   </c:if>
							   <c:if test="${param.order!='fdname'}">
								<button class="btn btn-large" type="button" onclick="findeCoursesByKey('1','fdname')">导师</button>
							   </c:if>
	...
	<section class="section listWrap">
		<ul class="nav list">
			 <j:iter items="${page.list}" var="bean" status="vstatus">
				 <li><a href="${ctx}/course/getSingleCourseAuthInfo?teacherId=${bean.FDID}&fdType=15&order=createtime"><i class="icon-exam-num"></i>
				
				    <span class="title">
					${bean.FDNAME}（${bean.FDEMAIL}）&nbsp;&nbsp;${bean.DEPTNAME}
					（课程：${bean.COURSES}人，新教师：${bean.TEACHERS}人）
					</span> 
...
function exportData(){
	...
	jalert("您确定要导出全部数据吗？",function(){
		  window.location.href="${ctx}/common/exp/getExpAllTutorAuth?order="+order+"&fdTitle="+fdTitle;
	}); 
}

//新增   
function addAuth(){
	window.location.href="${ctx}/course/getSingleCourseAuthInfo?teacherId=&fdType=15&order=createtime";
}
</script>
<script type="text/javascript">	
