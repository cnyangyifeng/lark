...
!function($){
    ...
    $.fn.loadJS = function(opt){
        ...
        if(s.name){
            var self = this;
            var src = s.path + s.name;
            var html = $('<script src="' + s.path + s.name + '" type="text/javascript"><\/script>'),
                flag = true;
            if(s.afterSelector){
                if($(s.afterSelector).next().attr("src") != src){
                    $(s.afterSelector).after(html);
                }
            } else {
    ...
    $.fn.goTop = function(options){
        ...
        if(isExists===true){
            ...
            node.bind('click',function(){
                $('html, body').animate({
                    scrollTop: 0
                },options.scrollSpeed==0?0:options.scrollSpeed,"swing")
            })
...
$("#topNav").each(function(){
    var $this =  $(this);
    var left = $this.children(".active").length ? ($this.children(".active").index() -1) * 104 : 0;
    var $bg = $this.children(".itemBg").css("left",left);
    $this.children("li").not(".itemBg,.specific").each(function(i){
        $(this).hover(function(){
            $bg.stop(true,false).animate({"left":(i+1)*104},200,"swing");
        },function(){
            $bg.stop(true,false).animate({"left":left},"swing");
        })
    })
});
function jalert(str,call){
    $.fn.jalert({
        title: "提示",
        content: str,
        callback: call
...
function jalert_tipst(str,time){
    $.fn.jalert({
        title: "提示",
        content: str,
        tip: true,
